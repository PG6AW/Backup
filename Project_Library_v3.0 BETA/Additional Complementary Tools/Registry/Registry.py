...
def register_user():
    ...
    cursor1.execute("INSERT INTO registered_users (username, name, middle_name_last_name, age, address, phone_number, national_id, email, postal_code, event_by_admin, sub_admin, event_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (username, name, middle_name_last_name, age, address, phone_number, national_id, email, postal_code, event_by_admin, the_user, datetime.datetime.now()))
    conn1.commit()
    conn1.close()
...
def register_book():
    ...
    cursor1.execute("INSERT INTO registered_books (category, book_id, title, author, publisher, year_of_publish, isbn, pages, translated_by, book_genre, description, event_by_admin, sub_admin, event_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (category, book_id, title, author, publisher, year_of_publish, isbn, pages, translated_by, book_genre, description, event_by_admin, the_user, datetime.datetime.now()))
    conn1.commit()
    conn1.close()
...
def delete_book():
    ...
    cursor1.execute("INSERT INTO deleted_registered_books (book_title, event_by_admin, sub_admin, event_date) VALUES (?, ?, ?, ?)",
                (booktitle, event_by_admin, the_user, datetime.datetime.now()))
    conn1.commit()
    conn1.close()
...
def delete_user():
    ...
    cursor1.execute("INSERT INTO deleted_registered_users (username, event_by_admin, sub_admin, event_date) VALUES (?, ?, ?, ?)",
                (username, event_by_admin, the_user, datetime.datetime.now()))
    conn1.commit()
    conn1.close()
...
def treeview():
    ask = messagebox.askyesno("confirmation", "You are opening the library registry environment in a treeview and read-only mode!\n\n__PROCEED?__")
    if ask:
        subprocess.Popen(['python', 'Additional Complementary Tools/Registry/Treeview.py'])
    else:
        return
