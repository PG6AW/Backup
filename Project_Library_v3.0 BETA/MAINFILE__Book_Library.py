# The Main Library File #requires login to work
...
cursor1.execute("INSERT INTO admin_enter_exit_log (admin, sub_admin, admin_enter_date) VALUES (?, ?, ?)",
            (event_by_admin, the_user, datetime.datetime.now()))
conn1.commit()
conn1.close()
...
def add_book():
    ...
    try:
        ...
        conn1.commit()
        cursor1.execute("INSERT INTO books_added (book_id, book_title, book_author, event_by_admin, sub_admin, event_date) VALUES (?, ?, ?, ?, ?, ?)",
                    (book_id, book_title, book_author, event_by_admin, the_user, datetime.datetime.now()))
        conn1.commit()
        conn1.close()
...
def exit_the_program():
    confirm = messagebox.askyesno("Confirmation", "Are you sure you're going to log out of your account and quit the program now?")
    if confirm:
        pass
    else:
        return
    ...
    try:
        ...
        if user_name is None:
            ...
            for the_user in user_name:
                the_user = str(the_user)
        cursor2.execute("INSERT INTO login_logs (name, username, email, event_by_admin, login_date) VALUES(?, ?, ?, ?, ?)", (f"@ {the_user}", "LOGGED->OUT", "@@@", event_by_admin, f"LOGOUT {datetime.datetime.now()}"))
        conn2.commit()
        conn2.close()
    except (sqlite3.OperationalError , ValueError , sqlite3.ProgrammingError):
        pass
    conn1 = sqlite3.connect("event_logs.db")
    cursor1 = conn1.cursor()
    cursor1.execute("INSERT INTO admin_enter_exit_log (admin, sub_admin, admin_exit_date) VALUES (?, ?, ?)",
                (event_by_admin, the_user, datetime.datetime.now()))
    conn1.commit()
    conn1.close()
    try:
...
def add_user():
    ...
    cursor1.execute("INSERT INTO user_signup (username, password, event_by_admin, sub_admin, event_date) VALUES (?, ?, ?, ?, ?)",
                (username, password, event_by_admin, the_user, datetime.datetime.now()))
    conn1.commit()
    conn1.close()
...
def delete_book():
    ...
    try:
        ...
        cursor1.execute("INSERT INTO deleted_books (book_title, book_id, event_by_admin, sub_admin, event_date) VALUES (?, ?, ?, ?, ?)",
                    (book_title, book_id, event_by_admin, the_user, datetime.datetime.now()))
        conn1.commit()
        conn1.close()
...
def borrow_book():
    ...
    cursor1.execute("INSERT INTO borrowed_books_by_users (user_id, username, book_id, book_title, days_period, event_by_admin, sub_admin, event_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (user_id, username, book_id, book_title, days_period, event_by_admin, the_user, datetime.datetime.now()))
    conn1.commit()
    conn1.close()
...
def return_book():
    ...
    conn1.commit()
    cursor1.execute("INSERT INTO returned_books (username, book_title, book_id, reserved_state, event_by_admin, sub_admin, event_date) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (username, book_title, book_id, rd, event_by_admin, the_user, datetime.datetime.now()))
    conn1.commit()
    conn1.close()
    ...
    if reserved_deletion:
        try:
            ...
            if i != "Reserved":
                ...
                cursor1.execute("INSERT INTO reserved_deletion_log (reserved_book_title, reserved_book_id, event_by_admin, sub_admin, event_date) VALUES (?, ?, ?, ?, ?)",
                            (i, book_id, event_by_admin, the_user, datetime.datetime.now()))
                conn1.commit()
                conn1.close()
                book_id_entry_delete.delete(0, tk.END)
            else:
    ...
    if reserved_deletion:
        try:
            ...
            if i == "Reserved":
                ...
                cursor1.execute("INSERT INTO unreserved_deletion_log (unreserved_book_title, unreserved_book_id, event_by_admin, sub_admin, event_date) VALUES (?, ?, ?, ?, ?)",
                            (i, book_id, event_by_admin, the_user, datetime.datetime.now()))
                conn1.commit()
                conn1.close()
                book_id_entry_delete.delete(0, tk.END)
